...
class ProcessingBase(ast.NodeVisitor):
    ...
    def _visit_assign(self, value, targets):
        ...
        def do_assign(decoded_items, target):
            decoded = decoded_items[0]
            decoded_fs = decoded_items[1]
            self.visit(target)
    ...
    def decode_node(self, node):
        def _get_fs_lineno(self, decoded, value):
        ...
        if isinstance(node, ast.Name):
            to_return = [self.scope_manager.get_def(self.current_ns, node.id)]
            fs_return = _get_fs_lineno(self, to_return, node)
            return (to_return, fs_return)
        elif isinstance(node, ast.Call):
            decoded = self.decode_node(node.func)
            return_defs = []
            for called_def in decoded:
                called_def = called_def[0]
